...
struct thread * get_thread(tid_t num);
struct thread * get_thread_all(tid_t num);
...
/* Looks up and returns pointer of thread with tid == num
   from the ready_list */
struct thread *
get_thread(tid_t num)
{
  struct list_elem * e;
